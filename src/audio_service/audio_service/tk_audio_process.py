...
from audio_service.ollama_client import OllamaChatClient
...
class AudioProcess(Node):
    def __init__(self):
        ...
        self.xunfei_text_read_thread = None
    ...
    def on_asr_sentence(self, msg: String):
        ...
        if self.audio_player.is_speaking() and wake_up:
            ...
            self.get_logger().info(f"收到[{msg.data}]包含唤醒词，停止天工行者说话")
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                try:
                    ...
                    count = 0
                    for chunk in self.ollama_client.stream_sentence(process_question):
    ...
    def close(self):
        ...
        if self.xunfei_text_read_thread and self.xunfei_text_read_thread.is_alive():
            self.xunfei_text_read_thread.join(timeout=1)
