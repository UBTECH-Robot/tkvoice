...
| 项目 | 需求 | 说明 |
|------|------|------|
| 操作系统 | Ubuntu (Arm64) | Orin 板运行 |
| Python | 3.10.12(Orin板上的默认版本) | 用于运行应用和安装包 |
| ROS 2 | Humble | 中间件框架 |
| 网络 | 局域网连接 | Orin 与 x86 能互相访问 |
...
```
工作目录/
├── tkvoice_release_0.2.26_1201_164512.tar  (安装包)
├── install.sh                              (本脚本)
└── ... (脚本会解压出来的目录)
```
...
- [ ] x86 服务器（192.168.41.1）已安装 Docker
- [ ] x86 服务器 SSH 服务已启动，可以用 `ssh ubuntu@192.168.41.1` 连接
- [ ] Orin 板已安装 ROS 2 Humble
- [ ] Orin 板与 x86 网络可达（可 ping 通）
- [ ] Orin 板上当前用户有 `sudo` 权限
- [ ] 足够的磁盘空间（`df -h` 检查）
...
RELEASE_DIR="tkvoice_release_0.2.26_1201_164512"  # 解压目录名
```
