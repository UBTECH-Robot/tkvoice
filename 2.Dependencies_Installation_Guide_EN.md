...
|------|-------------|-------------|
| Operating System | Ubuntu (Arm64) | Running on Orin Board |
| Python | 3.10.12 (Default on Orin) | For running applications and installing packages |
| ROS 2 | Humble | Middleware framework |
| Network | Local Area Network | Orin and x86 must be able to communicate |
...
└── ... (Directories that will be extracted by the script)
```

### Pre-Installation Checklist

- x86 Server (192.168.41.1) has Docker installed
- x86 Server SSH service is running and can be connected with `ssh ubuntu@192.168.41.1`
- Orin Board has ROS 2 Humble installed
- Orin Board and x86 network are reachable (can ping)
- Current user on Orin Board has `sudo` privileges
