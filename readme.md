...
1. 先从本地电脑上将 tkvoice_release_0.3.6_0313_170837.tar 传到 41.2 的 Orin 板：
```bash
...
2. 登录到 41.2 的 Orin 板后，先只解压 tkvoice_release_0.3.6_0313_170837.tar 内的安装脚本 install.sh，其他的解压工作由安装脚本按需完成：
```bash
...
# 再注意，执行uninstall.sh 后 ~/tkvoice_release_0.3.6_0313_170837 目录会删除掉，也就是卸载脚本自身也会删除掉，但是 ~/tkvoice_release_0.3.6_0313_170837.tar 文件不会删除
```
